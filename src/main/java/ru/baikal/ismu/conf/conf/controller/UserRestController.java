        ...
        if (!userRepo.findByUserEmail(user.getUserEmail()).isPresent()) {
            ...
            notification.setNotificationBody(
                    "Здравствуйте," + user.getLastname() + " " + user.getFirstname() + " " + user.getSecname() + "! \n" +
                            ...
                            "Для просмотра докладов Вам необходимо перейти в раздел \"Доклады\", авторизовавшись со следующими учетными данными:" +
                            "\n Логин: " + user.getUsername() + " " +
                            "\n Пароль: " + user.getPassword() + " " +
                            "\n \n Это письмо было отправлено Вам, так как адрес " + user.getUserEmail() + " был указан при регистрации на https://stom.ismu.baikal.ru" + " " +
                            "\n \n Для отказа от получения уведомлений перейдите по ССЫЛКЕ: https://stom.ismu.baikal.ru/unsubscribe/" + user.getNotificationUUID());
