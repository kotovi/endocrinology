...
@RestController
@RequestMapping("/agreeTickets")

public class AgreeTicketRestController {
    @Autowired
    TezisRepo tezisRepo;
    @Autowired
    DeskRepo deskRepo;
    ...
    public List<Tezis> tezisList(@AuthenticationPrincipal User user){
        if (user.getRoles().contains(Roles.ADMIN)){
            return tezisRepo.findAllByTezisStatus(1);
            ...
            if (deskIds.size()>0) {
                ...
                return tezisRepo.findAllByTezisStatusAndDeskIdIn(1, deskIds);
            } else return null;
