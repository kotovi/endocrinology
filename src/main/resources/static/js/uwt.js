...
Vue.component('agree-row' , {
    ...
    template:
        ...
        '<li class="list-group-item"><b>Ученая степень, ученое звание, должность:</b> {{agree.degree}}</li>'+
        '<li class="list-group-item"><b>Организация:</b> {{agree.organization}}</li>'+
        '<li class="list-group-item"><b>Адресс рганизации:</b> {{agree.organizationPostAddress}}</li>'+
...
Vue.component('agrees-list', {
    ...
    template:
        ...
        '<h1 class="display-4 mt-5 mb-5" style="padding-top:40px;">Пользователи без заявок</h1>'+
...
app = new Vue({
    ...
    created: function () {
        axios.get('/userswithouttezis').then(result => {
